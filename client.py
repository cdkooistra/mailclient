...
class imapClient:
    ...
    def __init__(self, recipient, password, server, use_ssl=True, move_to_archive=True):
        if not recipient:
        ...
        if mbox_response == 'OK':
            for num in msgnums[0].split():
                ...
                if isinstance(subject[0], bytes):
                    msg_subject=subject[0].decode(encoding='UTF-8')
                else:
                    msg_subject=subject[0]
...
class smtpClient:
    ...
    def __init__(self, sender, password, server, port, use_ssl=True):
        if not sender:
    ...
    def send_msg(self, receiver, msg_subject, msg_body):
        try: 
            msg = MIMEMultipart()
